...
describe('пустое хранилище и полнота', () => {
  it('пустое хранилище: объяснение вместо аварии, запуск недоступен', async () => {
    ...
    // (FR-026). Запуска нет нигде — в макете действия не дублируются, и
    // предлагать сбор там, где собирать не из чего, нечестно.
    ...
    // Объясняют обе панели: чего не хватает и кто это делает.
    expect(screen.getByText(/Сначала нужна первичная загрузка/)).toBeInTheDocument();
    expect(screen.getByText(/Первичную загрузку выполняет администратор/)).toBeInTheDocument();
  });
