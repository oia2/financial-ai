 ...
 *  - **пауза не останавливает сбор данных.** Это разные механизмы, и слитое
    ...
    (paused: boolean) => {
      setPaused.mutate(paused, {
        onSuccess: () =>
          setNotice(
            paused
              ? 'Автоматические запуски ранжирования выключены. Сбор рыночных данных продолжается.'
              : 'Автоматический режим возобновлён.',
          ),
        onError: (error) => setNotice(refusalMessage(error)),
      });
    },
