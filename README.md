...
| Проверка на реальном брокере | ✅ пройдена: счёт синхронизируется, суммы сходятся с брокером |

Стек собирается и поднимается целиком, все сервисы отвечают healthy, состояние счёта
читается из T-Bank Invest API на живых данных. Отображаемая общая стоимость совпадает с
итогом брокера, сумма долей даёт ровно 100%.

Токен задаётся в `deployments/docker-compose/.env` **без кавычек**: значение в кавычках
уходит к брокеру вместе с ними и отклоняется как недействительное.
