...
| Проверка | Значение |
|---|---|
| Тесты | 158 backend, 142 frontend |
...
На **пустом** хранилище догон не работает намеренно: это не дыра, а отсутствие истории, и
закрывать её должна первичная загрузка `backfill` — она берёт историю по бумаге целиком,
что дешевле перебора дат на всю глубину окна.
