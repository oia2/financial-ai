...
| Код | ✅ реализован: backend, frontend, миграции, docker compose |
| Тесты | ✅ 152 backend, 68 frontend; все quality gates зелёные |
| Проверка на реальном брокере | ⚠️ упирается в токен: T-Bank отвечает `UNAUTHENTICATED` |

Стек собирается и поднимается целиком, все сервисы отвечают healthy. Единственное, что
не проверено на живых данных, — сценарии, требующие успешного ответа T-Bank Invest API:
текущий токен в `deployments/docker-compose/.env` брокер отклоняет, его нужно перевыпустить
с правами только на чтение. Система при этом ведёт себя как задумано: показывает состояние
«Брокер не подключён», не подменяет данные нулями и не раскрывает токен.
