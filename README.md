...
| `MARKET_DATA_*` | backend-worker | Сбор рыночных данных MOEX: включение, время запуска после закрытия сессии, глубина первичной загрузки, глубины окон, срок хранения наборов, догон пропущенных сессий. Секретов среди них нет — данные MOEX ISS публичны |
...
Если worker не работал несколько дней, пропущенные сессии он **догоняет сам**: сравнивает
торговый календарь с собранным и добирает недостающее в пределах окна набора. Порога по
числу пропущенных дней нет — система работает с 314 сессиями за раз, и остановка на
десятом дне сделала бы механизм бесполезным ровно в том случае, ради которого он заведён.

Посмотреть и запустить вручную:

```bash
docker compose … exec backend-worker python -m financial_ai.market_data.cli gaps
docker compose … exec backend-worker python -m financial_ai.market_data.cli catchup --dry-run
```
