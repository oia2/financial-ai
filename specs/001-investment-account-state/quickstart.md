...
Смоделировать недоступность брокера, подменив адрес T-Invest API на заведомо
недостижимый. Контейнер работает от непривилегированного пользователя, поэтому
править `/etc/hosts` внутри него нельзя — недоступность задаётся конфигурацией:

```bash
cd deployments/docker-compose
TBANK_INVEST_TARGET=127.0.0.1:1 docker compose up -d backend-worker
```

Вернуть рабочий адрес: `docker compose up -d backend-worker` без переменной.
...
```bash
cd deployments/docker-compose
TBANK_INVEST_READ_TOKEN= docker compose up -d backend-worker
```

Вернуть токен: `docker compose up -d backend-worker` без переменной.
...
Оставить брокера недоступным (см. 4.5) при наличии сохранённого состояния и
подождать `max(3 × интервал, 180 с)` — при интервале 60 с и меньше это ровно
180 секунд.
