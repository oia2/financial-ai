...
- [ ] T047 [P] Обновить `README.md` и `AGENTS.md`: состав источников без дивидендов, объявление календаря, новые таблицы
- [ ] T048 [P] Обновить `backend/src/financial_ai/market_data/PROVENANCE.md`: что перенесено из исследовательского репозитория в этой фиче и что расходится сознательно
- [ ] T049 Сверить раздел с макетом: запустить `verify.cjs` и `check-width.cjs` из `design-assets/market-data-collection-v4/` проекта Open Design и сопоставить состояния с [contracts/ui-states.md](./contracts/ui-states.md)
