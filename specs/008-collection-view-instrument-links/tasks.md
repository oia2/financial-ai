...
- [X] T225 Обрабатывать точные оставшиеся пары ремонта; завершённый план повторять без запросов и изменения исхода.
- [X] T226 Проверить сохранение бюджета, мягкую остановку и продолжение ремонта после холодного рестарта.
- [X] T227 Ускорить сводку без изменения смысла; подтвердить SQL и время сопоставимыми измерениями.
- [X] T228 Объединить обход доски для выбранных котировок и агрегатов с сохранением отдельных результатов.
- [X] T229 Проверить ускорение Brent по историческому каталогу либо зафиксировать доказанный предел рабочего fallback.
- [X] T230 Выполнить read-only аудит актуального окна и подготовить конкретный объём, оценку и бюджет ремонта.
- [X] T231 Выполнить только разрешённый ремонт и сопоставить БД, сводку, журнал, готовность ML и набор.
- [X] T232 Выполнить полный scripts/check.sh, живые сверки и визуальную приёмку, сохранив ограничения отдельно.
- [X] T233 Актуализировать документацию и подтвердить итог каждого замечания доказательствами; не скрывать незавершённую приёмку.

## Phase 49: Восстановление работоспособности по обращению 2026-09-22

- [X] T234 Устранить задержку чтения на накопленной базе; разделить расписание новых дат и повтор старых; объяснить неподтверждённую историю и вернуть явный ручной добор. Исправить получение Brent, подтвердить полным гейтом и живым запуском обновлённого стенда без браузера. Доказательства: `remediation-results-v2.md`, раздел T234.

## Phase 50: «Один раз собрать, дальше добавлять» — анализ и обращение 2026-09-23

[Анализ](./analysis-2026-09-23.md) подтвердил дефекты A1–A7; сверх него найдено, что
22.09 календарь спросили после порога один раз и больше не переспрашивали. Требования —
FR-032e/f, FR-033c–i, FR-040 (уточнение), FR-024d–f. Отметки Phase 47–49 готовность не
подтверждают.

- [X] T235 Зафиксировать решения владельца 2026-09-23 в `spec.md`, `data-model.md` (`failure_kind`) и контракте сводки.
- [X] T236 A1: проверять содержимое ответа — идентификатор и разбор чисел — до выдачи доказательства; нарушение контракта не превращать в `NULL`/пропуск (`equity_d1.to_decimal`, `rows_to_bars`, агрегаты, глобальные ряды, Brent, ЦБ).
- [X] T237 Закрывать сессию по доказательствам единиц работы, а не по статусу прогона (FR-032f); диапазон закрывает доказанные даты.
- [X] T238 A2: планировать и исполнять пары «источник — дата» в собственном окне источника в ручном сборе, продолжении и автоматическом добор.
- [X] T239 A4: продолжать глобальные ряды и ЦБ по недоказанному остатку.
- [X] T240 A5: сохранять итог сессии ежедневного сбора и его продолжения так же, как ручного.
- [X] T241 A6: хранить и отдавать `failure_kind` (миграция с разметкой старых записей); `running` не считать неудачей.
- [X] T242 A7: обновлять выбранные справочники в ручном сборе один раз за запуск.
- [X] T243 A3: первичная загрузка на пустой базе и после обычного сбора; ручной сбор окна на пустой базе; верхняя граница — последняя закрытая сессия.
- [X] T244 FR-040: переспрашивать календарь после порога, пока сегодняшней даты нет.
- [X] T245 Экран: состояния FR-024e, подпись «Данные по», без шаблонных пояснений и лишних счётчиков — сначала в Open Design, затем в коде.
- [X] T246 Регрессии на все пункты: испорченный ответ, окна разной длины (ноль запросов позиций вне окна), продолжение диапазона, журнал ежедневного сбора после рестарта, причины незавершённости, справочники, загрузка с нуля, календарь.
- [X] T247 Полный `scripts/check.sh`; актуализация README/AGENTS при изменении команд.
- [X] T248 Однократный проход по окнам групп на рабочей базе (FR-033i); сверка сводки, журнала и готовности ML; результат — в `analysis-2026-09-23.md`, раздел «Исправление». Выполнено 2026-09-23 после T250: все группы 314/314, позиции 82/82, справочники проверены.
- [X] T249 По ходу прохода: сводка групп перечитывается во время сбора; источник вне своего окна не входит в ленту и счётчик сессии; фраза «— на весь период, — раз в сутки» убрана из макета и кода; счётчик ленты считает ровно показанные строки; справочник, проверенный сегодня, ручным сбором повторно не спрашивается (замечания владельца 2026-09-23).
- [X] T250 Позиции одним CSV-запросом на дату (FR-053a) и неприменимость до начала торгов семейства (FR-032g); прежний обмен формой по контракту удалить; регрессии; перезапуск сбора.

## Phase 51: Ревью готовности модуля 2026-09-23

Ревью ревизии `1a6af62`: окно собрано (314/314, 82/82), но вход модели содержит старые
позиции `unknown`, неопубликованная таблица позиций исчерпывает попытки сессии, а
справочники не проверяются готовностью ML. Требования — FR-032h, FR-033j, FR-040a, FR-051a.

- [X] T251 Зафиксировать FR-032h, FR-033j, FR-040a, FR-051a в `spec.md`, `data-model.md` и контракте сводки.
- [X] T252 FR-051a: исключить позиции `unknown` из набора для модели; регрессия.
- [X] T253 FR-032h: `failure_kind = unpublished`; не кешировать неопубликованную таблицу; не повторять задержанный источник в том же прогоне; не считать такой прогон попыткой сессии; регрессии.
- [X] T254 FR-033j: готовность ML проверяет справочники по проверенному успеху; регрессия.
- [X] T255 FR-040a: московская верхняя граница календаря.
- [X] T256 Ускорить единый гейт без исключения проверок; полный `scripts/check.sh`. Выполнено: backend-тесты по 6 шардам со своей БД, эмулятор и frontend параллельно; полный гейт 152 с (было ~6 мин), 939 + 61 + 238 тестов, сборка образов прошла. `pytest-xdist` не добавлен: индекс T-Bank не проходит проверку сертификата при разрешении `uv.lock`.

## Phase 52: Пустой свежий ответ — не отсутствие (повторное ревью 2026-09-23)

Повторное ревью `01e33e8`: Brent, состав индекса и ключевая ставка ЦБ записывали пустой
ответ подтверждённым отсутствием, глобальные ряды — для всех дат, кроме сегодняшней.
Требование — FR-032i.

- [X] T257 Зафиксировать FR-032i в `spec.md`.
- [X] T258 FR-032i: Brent, состав индекса, ставка ЦБ и ряды ISS — пустой ответ даёт `unpublished` без доказательства; строки без применимого значения — `source`; регрессии, включая пересмотр тестов прежнего правила.
- [X] T259 Полный `scripts/check.sh`, перезапуск стенда.

## Phase 53: Внешнее ревью 2026-09-23

Ревью `7f00bb2` подтверждено по коду: попытки расходуются прогрессом, справочники не
повторяются без сессионной работы, пустой перечень лотов — ложный успех, первый ручной
сбор пустой базы не строит связи и лоты, журнал переносит неуспех диапазона на
доказанные даты. Автоматическое восстановление истории по одной дате (п. 6) — отдельная
задача производительности, не входит в фазу. Требования — FR-032j, FR-033k–n.

- [X] T260 Зафиксировать FR-032j, FR-033k–n в `spec.md`.
- [X] T261 FR-032j: предел попыток считает только отказы `source`/`internal`; регрессия постепенной публикации.
- [X] T262 FR-033k: ежедневный цикл повторяет непроверенный сегодня справочник без сессионной работы, с выдержкой; регрессия.
- [X] T263 FR-033l: пустой перечень лотов — отказ; регрессия.
- [X] T264 FR-033m: ручной сбор пустой базы — котировки последней сессии до связей, справочники после цикла; регрессия «связи и лоты за один проход».
- [X] T265 FR-033n: итог даты диапазонного источника по доказательствам; регрессия.
- [X] T266 Полный `scripts/check.sh`, перезапуск стенда.
- [X] T268 FR-040b: календарь переспрашивается, пока закрытый будний день после последней сессии не подтверждён; регрессия ночи 24.09.
- [X] T269 FR-024g: журнал — только прогоны; убрать «Пропущенные сессии», «Состав инструментов» и подпись — сначала макет Open Design (`v4.js`, `v4.css`, `verify.cjs`), затем `RunJournal`, `design.css`.
- [X] T267 FR-054a: после порога без опубликованного дня — «ждём публикации биржей»; сначала макет Open Design (`v4.js`, `verify.cjs`), затем `coverage.py` и оба виджета.

## Phase 54: Порог по времени публикации итогов дня

- [X] T270 FR-040c: порог по умолчанию 23:59 МСК — конфигурация, compose, `.env.example`, спецификация, документация; регрессия «после полуночи не останавливается» для 23:59; перезапуск стенда.
- [X] T271 FR-054b: «сегодня/завтра» по местной дате момента сбора — сначала макет Open Design (`v4.js`, `verify.cjs`), затем `formatCollectionStart` и оба виджета; регрессии для Красноярска и Москвы.

## Phase 55: Ревью готовности 2026-09-24 — акции и фонды, календарь при отказе

Ревью `62c16e4`: с 22.06.2026 биржа торгует фондами на TQBR, и 243 фонда шли во вход
модели как акции; при отказе биржи календарь спрашивался на каждом тике; SC-010
расходился с FR-040c. Требования — FR-040d, FR-060–FR-060e, SC-010 (уточнение).

- [X] T272 Зафиксировать FR-040d, FR-060–FR-060e, уточнение SC-010 в `spec.md`, `security_kind` в `data-model.md`, группы фондов и `model_input` в контракте сводки.
- [X] T273 FR-040d: неудачный запрос календаря выдерживает интервал повтора; регрессия «биржа недоступна сутки».
- [X] T274 FR-060: миграция `0020` (`market_asset.security_kind`); справочник бумаг одним обращением получает лоты, ISIN и вид, для бумаг вне списка — описание; нераспознанный вид — отказ; регрессии.
- [X] T275 FR-060a/b: группы `fund_quotes`, `fund_aggregates` с окном от 22.06.2026; строки групп по виду бумаги; окно источника — объединение окон его групп; регрессии.
- [X] T276 FR-060c/d/e: набор модели — только акции; справочник не проверен, пока у бумаги последней сессии нет вида; состав сводки — по акциям; регрессии.
- [X] T277 Экран: группы фондов в сводке и календаре, подпись «пока не входит в модель» в существующем подзаголовке группы (новых элементов нет); регрессия. Макет Open Design обновлён тем же днём, когда сервер снова подключился: группы фондов с подписью, шесть точек календаря по группам с историей, фонды в сведениях о дате; новая проверка в `verify.cjs` — 31 из 31.
- [X] T278 Полный `scripts/check.sh`, README/AGENTS, перезапуск стенда, проверка вида бумаг на рабочей базе. Выполнено 2026-09-24: гейт — 967 + 61 + 242 теста, образы собраны; после перезапуска справочник бумаг первым тиком проставил вид всем 513 бумагам с историей (268 акций, 245 фондов; снятые с торгов — по описанию); сводка: акции 314/314, фонды 68/68 с 22.06, состав 256 акций; готовность ML на 23.09 — `ready`. В окне модели теперь 267 акций вместо 512 бумаг; наборы за 22.09 и 23.09 собраны до исправления и содержат фонды.
- [X] T279 Источник доски назван по доске: «Котировки доски TQBR», «Агрегаты доски TQBR» (FR-060a, замечание владельца 2026-09-24: внутри группы фондов стояло «Котировки акций») — сначала макет, затем `plan.py`, контракт, тесты.
- [X] T280 Сводка читает журнал исходов и доказательства один раз на все группы, а не на каждую: с группами фондов журнал окна (~6000 записей) читался шесть раз, и открытие раздела занимало 1,4–1,8 с. Закрытые даты считает база (`sessions_with_all_work`: `GROUP BY … HAVING` по обязательным единицам) — правило полноты прежнее и одно для сводки, сбора и готовности; строки группы считаются одним проходом. Затем последний исход пары «сессия — источник» выбирает база (`latest_unfinished_runs`: `DISTINCT ON`, диапазонные прогоны — по торговым сессиям периода, наружу только незавершённые), а строки групп считаются одним проходом по таблице с разбивкой «дата × вид бумаги» (`rows_by_session`); эквивалентность прежним реализациям — `test_coverage_queries.py` и сверка на рабочей базе (400 сессий). Итог: построение сводки 0,16 с, ответ API 0,20–0,25 с (было 1,4–1,8 с); сводка до и после совпадает полностью.

## Phase 56: Ревью готовности 2026-09-24 (второй проход) — журнал, вид бумаги, выдержка

Ревью рабочего дерева после T280: прогон одного справочника показывался в журнале
ожидающей сессией; бумага без вида держала модель по всей истории, а не по дате решения;
внутри сбора сессии справочник не выдерживал повтор. Решение владельца 2026-09-24: снятая с
торгов бумага без вида выходит из набора с записью в манифест. Требования — FR-055a,
FR-056b, FR-060 (уточнение), FR-060d (уточнение), FR-060f.

- [X] T281 Зафиксировать FR-055a, FR-056b, FR-060f, уточнения FR-060 и FR-060d в `spec.md`; `excluded_assets` в контракте запроса ранжирования (spec 003).
- [X] T282 FR-056b: журнал считает сессиями только даты посессионных и диапазонных источников (`plan.DAILY_SOURCES`); регрессия «прогон одного справочника — 0 сессий, не ограниченная запись».
- [X] T283 FR-060d: готовность держит дату решения, только если в неё торговалась бумага без вида (`sessions_with_kindless_assets`); справочник перезапрашивается раньше суток только ради бумаги последней собранной сессии; регрессии.
- [X] T284 FR-060f: набор исключает снятую бумагу без вида, пишет `excluded_assets` в манифест и (непустой) в дайджест; бумага без вида, торговавшаяся в дату решения, — отказ набора; регрессии, в том числе «без исключений дайджест прежний».
- [X] T285 FR-060: нераспознанный вид — отказ справочника только у бумаги последней сессии; у снятой — запись в журнале сборщика; сломанное описание одной бумаги не обрывает разбор остальных; регрессии.
- [X] T286 FR-055a: выдержка повтора справочника общая для тика и сбора сессии (`reference_backs_off`); регрессия.
- [X] T287 T072 (spec 005) отмечена: фильтр по текущей базе в `sync/advisory.py::is_held` внесён в T234.


## Phase 57: Ревью готовности 2026-09-24 (третий проход) — R1–R5

Отчёт `review-2026-09-24.md` на `80502c3`: прямой сбор незакрытой сессии, ложная полнота
обязательных глобальных рядов и ЗКЦ, исключённая бумага в файлах набора, backfill без
охваченного периода, продолжение без справочников. Требования — FR-032k, FR-032l, уточнения
FR-060f, FR-033h, FR-058b.

- [X] T288 Зафиксировать FR-032k, FR-032l и уточнения FR-060f, FR-033h, FR-058b в `spec.md`.
- [X] T289 R1 / FR-032k: гейт закрытости в `ingest._ingest_session` до запросов наблюдений; `run` без даты берёт последнюю закрытую сессию; регрессии «явная дата в 10:00 — ничего не записано, после порога — собрана» и «без даты — предыдущая сессия».
- [X] T290 R2 / FR-032l: строки рядов ISS сверяются с запрошенным `SECID`; доказательство — только датам со значением; ставка и каждая обязательная точка ЗКЦ — со значением; тестовые подделки ISS отдают `SECID`; регрессии.
- [X] T291 R3 / FR-060f: позиции и секторы набора отбираются по тому же составу, что котировки и агрегаты; регрессия читает записанные JSON-файлы набора.
- [X] T292 R4 / FR-033h: доказательство истории — ключ `range:<бумага>:<начало>` с концом диапазона в дате; догружается только непокрытый остаток; прежний ключ `ticker:` покрытием не считается; регрессии «точный повтор — ноль запросов», «ранний `--from`», «поздняя граница», «прежний формат».
- [X] T293 R5 / FR-058b: `CatchupState.unfinished_references` и `resumable`; продолжение справочников тем же `run_id` без переспроса доведённых; маршрут продолжает по `resumable`; регрессии состояния и маршрута.
- [X] T294 Условия закрытия ревью 2026-09-24. Выполнено 2026-09-24 на `9faf504`: полный `scripts/check.sh` — 993 + 61 + 242 теста, миграции, образы. Живые данные ISS (IMOEX, RTSI, RGBI, RVI, USD000UTSTOM) проходят новую сверку `SECID`; значения 22–23.09 совпадают с базой. Read-only аудит рабочей базы: доказательств глобальных рядов, ставки и ЗКЦ без значения — 0; доказательств и баров котировок/агрегатов раньше 23:59 МСК своей даты — 0; доказательств истории прежнего формата `ticker:` — 0 (повторной загрузки не будет); ремонт не нужен. Стенд перезапущен с новым кодом; `verify-cbr`, `verify-brent`, `verify-positions --ticker SBER`, `verify-reference` за 23.09 — код 0. Сводка: все группы 314/314, 82/82, 68/68; `gaps` — пропусков нет; журнал — последние прогоны завершены без неудач. Набор за 23.09: 267 акций, ни в одном файле нет бумаг вне состава манифеста; дайджест прежний, поэтому переиспользован каталог, собранный в 12:29 до FR-060f: в его манифесте нет поля `excluded_assets` (исключений нет). Визуальная приёмка: экран не менялся; объём приёмки — решение владельца.
