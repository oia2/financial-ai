...
async def read_state(request: Request) -> dict[str, object]:
    ...
    **worker'а**, и второй счёт по другой конфигурации разойдётся с первым.
    ...
    async with factory() as session:
        ...
        # Чего именно не хватает — говорится, а не подразумевается. Расчёт
        # готовности этот ответ уже содержит; «ожидаются данные» без ответа
        # «каких?» оставляет человека гадать.
        blocking: list[dict[str, str]] = []
        if ready is None and last_known is not None:
