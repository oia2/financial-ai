...
class MarketDataScheduler:
    ...
    async def _ingest_once(self) -> None:
        ...
        - **разрыв не нарезается на куски.** Не «по три сессии за тик»:
          нарезка — тот же автоматический догон, только растянутый на
          несколько минут, и человек о нём не просил. Разделение ровно одно, и
          оно по смыслу работы: свежая сессия собирается отдельно и первой,
          история — следующим проходом целиком и по порядку (FR-045). Глубина
          ограничена окном догона — дальше сессия до модели не доходит. Если
          предел всё же задан числом и разрыв его превысил, не собирается
          ничего, и дыра уходит человеку.
        """
        if self._paused:
            return
        ...
        def make_plan(days: list[dt.date], run_id: str) -> None:
            ...
            self._state = CatchupState(
                ...
                # Порядок сбора сохраняется отдельно: по нему продолжение
                # берётся за работу с того места, где прогон остановили
                # (FR-058g).
                order=list(days),
                date_from=min(days),
                date_till=max(days),
