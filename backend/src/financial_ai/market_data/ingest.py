...
async def _run_delayed_source(
    ...
    for attempt in range(1, DELAYED_SOURCE_ATTEMPTS + 1):
        ...
        if outcome.status == STATUS_STOPPED:
            ...
            return outcome
        if outcome.failure_kind == plan.FAILURE_UNPUBLISHED:
            # Биржа дату ещё не опубликовала, и через секунду не опубликует:
            # ожидание публикации — дело выдержки между прогонами, а не
            # немедленных повторов (FR-032h).
            return outcome
        logger.info(
