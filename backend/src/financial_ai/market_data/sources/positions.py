...
async def sync_positions(
    ...
    ``on_progress`` зовётся после каждого инструмента: единица обращения здесь
    мельче сессии, источник идёт минутами и десятками обращений, а на экране
    всё это время стояло одно слово «идёт» — долгий источник неотличим от
    зависшего (FR-058i).
    """
    ...
    stopped = False
    for position, ticker in enumerate(wanted, start=1):
