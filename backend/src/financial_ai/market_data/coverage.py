...
async def build_report(
    ...
    #
    # Берётся САМАЯ ПОЗДНЯЯ несобранная — та, которую сбор возьмёт первой.
    # Ежедневный цикл идёт от свежих сессий к старым (FR-045), а строка
    # продолжала называть самую раннюю, то есть ту, до которой сбор дойдёт
    # последней: при отставании в восемьдесят дней она обещала прошлогоднюю
    # дату там, где система собиралась взять вчерашнюю (FR-054).
    #
    ...
    next_session = max(pending) if pending else await calendar.latest_session(moscow_today())
