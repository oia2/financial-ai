...
async def advance(
    ...
    *,
    ...
    # Свежее — первым. Порядок «от старых к новым» верен для ручного догона:
    # там важно не оставить разрозненные дыры. Но ежедневный цикл существует
    # ради сегодняшних данных, и при отставании этот порядок отдаёт их
    # последними: на стенде 2026-09-18 собранное кончалось 11.09 при календаре
    # до 17.09.
    #
    # Ставить первой именно ПОСЛЕДНЮЮ закрытую сессию мало: её может не быть в
    # плане — например, её недавно пробовали и она ждёт выдержки. Тогда первой
    # должна идти самая свежая из недостающих, а не самая старая (FR-045).
